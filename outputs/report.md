...
本系统由以下四个阶段构成，图1为完整系统架构图：

![图1：系统架构图](figure_pipeline.png)
...
三条件 prompt 差异如下：
- **Baseline**：原始日记机械英译 + 像素风格约束，保留全部叙事细节（HR面谈经过、地铁、学习经历）
- **Partial**：`anger, frustration, shame, low valence, high arousal`——仅情绪标签，无视觉指引
- **Full**：`被粗暴否定的努力化作滚烫的羞耻感，无处可藏`——隐喻驱动，搭配灼热红与暗红色调、无文字指令

所有图像均使用 **ChatGPT Images 2.0**（gpt-image-2，OpenAI，2026年4月）生成，像素艺术风格通过 prompt 工程实现。

图2展示了 diary_5、diary_7、diary_9、diary_10 共四篇日记的三条件生成结果，覆盖愤怒、娱乐、焦虑、喜悦四类情绪，加粗边框标注各行最高 CLIP 分，读者可结合摘要自行评判：

![图2：多日记三条件生成结果对比（Qualitative Gallery）](figure_qualitative_gallery.png)
...
图3（热力图）呈现了全部 30 张图的 CLIP 分布，Partial 列整体最浅（均值最低），Full 列整体最深（均值最高）；D9 为唯一逆序点（Partial > Full），D4/D6/D7/D9/D10 的 Baseline 得分高于 Full。图3（柱状图 + 折线图）为均值与逐条日记趋势概览。

![图3：CLIP Score 热力图（10 篇日记 × 3 条件）](figure_clip_heatmap.png)

![图4：各条件 CLIP 均值及逐日记趋势](clip_analysis.png)

### 4.3 结果分析

**Full 条件在均值层面最优**：隐喻驱动的视觉设计为图像生成模型提供明确的构图与意象方向，使生成结果在均值层面与日记情绪语义最为对齐。需要指出，Full 与 Baseline 的均值差距仅为 +2.5%，且在逐篇比较中 5/10 篇 Baseline 得分更高，说明 Full 的优势体现在整体趋势而非每一样本。
