...
本研究存在以下方法论局限：(1) 图像均由同一操作者顺序生成（Baseline → Partial → Full），存在 prompt 写作熟练度提升导致的轻微操作者偏差；(2) CLIP ViT-B/32 对像素艺术风格的绝对评分偏低（~0.22），但条件间相对排名具有参考价值；(3) 样本量为 10 篇日记，结论的统计功效有限。
